var Scene_2Layer = cc.LayerColor.extend({
    walls:[],
    bonus:[],
    guns:[],
    bullets:[],
    ...
    init_wall:function() {
        ...
        this.walls[0] = new Wall(200,30);
        this.walls[1] = new Wall(30,140);
        this.walls[2] = new Wall(200,30);
        this.walls[3] = new Wall(30,40);
        this.walls[4] = new Wall(30,40);
        this.walls[5] = new Wall(300,40);


        // 设置墙的锚点位置
        this.walls[0].setPosition(cc.p(origin.x + winSize.width/2 , origin.y + winSize.height/2+this.walls[1].getContentSize().height/2+this.walls[0].getContentSize().height/2));
        this.walls[1].setPosition(cc.p(origin.x + winSize.width/2 -this.walls[0].getContentSize().width/2+this.walls[1].getContentSize().width/2, origin.y + winSize.height/2));
        this.walls[2].setPosition(cc.p(origin.x + winSize.width/2 , origin.y + winSize.height/2-this.walls[1].getContentSize().height/2-this.walls[2].getContentSize().height/2));
        this.walls[3].setPosition(cc.p(origin.x + winSize.width/2 + this.walls[0].getContentSize().width/2 - this.walls[3].getContentSize().width/2 , this.walls[2].getPosition().y + this.walls[2].getContentSize().height/2+this.walls[3].getContentSize().height/2));
        this.walls[4].setPosition(cc.p(origin.x + winSize.width/2 + this.walls[0].getContentSize().width/2 - this.walls[3].getContentSize().width/2 , this.walls[0].getPosition().y - this.walls[0].getContentSize().height/2-this.walls[4].getContentSize().height/2));
        this.walls[5].setPosition(cc.p(origin.x + this.walls[5].getContentSize().width/2,origin.y + winSize.height * 0.8));

        //移动墙壁
        this.walls[0].LRMove("left",70,2);
        this.walls[1].LRMove("left",70,2);
        this.walls[2].LRMove("left",70,2);
        this.walls[3].LRMove("left",70,2);
        this.walls[4].LRMove("left",70,2);
    ...
    init_bonus:function(){
        ...
        //设置小金块的位置
        this.bonus[0].setPosition(cc.p(50,665));
        this.bonus[1].setPosition(cc.p(origin.x + winSize.width/2,origin.y + winSize.height/2));
        this.bonus[2].setPosition(cc.p(origin.x+winSize.width*0.7,origin.y+winSize.height*0.1));

        //设置金块的移动
        this.bonus[1].LRMove("left",70,2);
        this.bonus[2].LRMove("left",300,5);
    ...
    init_sawt:function(){
        ...
        this.sawtes[0].setPosition(cc.p(this.walls[1].getPosition().x + this.walls[1].getContentSize().width/2,this.walls[0].getPosition().y - this.walls[0].getContentSize().height/2));
        this.sawtes[1].setPosition(cc.p(this.walls[1].getPosition().x + this.walls[1].getContentSize().width/2,this.walls[2].getPosition().y + this.walls[2].getContentSize().height/2));
        this.sawtes[0].LRMove("right",80,2);
        this.sawtes[1].LRMove("right",80,2);
    ...
    init_gun:function(){
        ...
        this.guns[0] = new Gun();
        this.guns[0].setPosition(cc.p(450,750));
    ...
    update:function(dt){
        ...
        if(this.bonus.length == 0){
            var success = new Scene_3();
            cc.Director.getInstance().replaceScene(cc.TransitionCrossFade.create(2, success));
        }
    }
});

var Scene_2 = cc.Scene.extend({
    onEnter:function () {
        this._super();
        var layer = new Scene_2Layer();
        this.addChild(layer);
        layer.init();
    }
});/**
